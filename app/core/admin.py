...
class FolderAdmin(ModelView, model=Folder):
    column_list = [Folder.id, Folder.name, Folder.virtual_path, Folder.parent_id, Folder.created_at]
    column_searchable_list = [Folder.name, Folder.virtual_path]
    # column_filters = [Folder.is_published]

class FileAdmin(ModelView, model=File):
    column_list = [File.id, File.name, File.mime_type, File.size_bytes, File.folder_id, File.created_at]
